## use confound cosine to model noise by linear regression and subtract to get cosed real activity
import os, json, time
...
for sub in all_subjects:
	...
	for run in range(1, total_run + 1):		
		...
		# get noise data
		noise_data = np.load(noise_dir + sub + '_run_' + str(run) + '_cos.npy') # t x c
		# get roi data
		...
		# linear regression on each voxel: cos -> roi pattern
		...
		for m in range(0, len(rois)):
			...
			out_file = sub_out_dir + sub + '_' + rois[m] + '_run_' + str(run) + '_decosed.npy'
