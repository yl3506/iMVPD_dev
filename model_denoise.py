...
for sub_1_index in range(0, len(all_subjects) - 1):
	for sub_2_index in range(sub_1_index + 1, len(all_subjects)):
		...
		for direction_index in range(0, 2): 
			...
			sub_1_data_dir = work_dir + sub_1 + '_complete/' + sub_1 + '_denoised/' 
			sub_2_data_dir = work_dir + sub_2 + '_complete/' + sub_2 + '_denoised/' 
				...
				for mask_2_index in range(mask_1_index + 1, len(all_masks)):
					...
					for mask_direction in range(0, 2):
						...
						for this_run in range(1, total_run + 1): 
							...
							test_1_dir = sub_1_data_dir + sub_1 + '_' + mask_1 + '_run_' + str(this_run) + '_real.npy'
							test_2_dir = sub_2_data_dir + sub_2 + '_' + mask_2 + '_run_' + str(this_run) + '_real.npy'
							...
							for run in it.chain(range(1, this_run), range(this_run + 1, total_run + 1)):
								if first_flag:
									train_1 = np.load(sub_1_data_dir + sub_1 + '_' + mask_1 + '_run_' + str(run) + '_real.npy')
									train_2 = np.load(sub_2_data_dir + sub_2 + '_' + mask_2 + '_run_' + str(run) + '_real.npy')
									first_flag = False
								else:
									train_1 = np.concatenate((train_1, np.load(sub_1_data_dir + sub_1 + '_' + mask_1 + '_run_' + str(run) + '_real.npy')))
									train_2 = np.concatenate((train_2, np.load(sub_2_data_dir + sub_2 + '_' + mask_2 + '_run_' + str(run) + '_real.npy')))
