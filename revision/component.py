...
work_dir = '/gsfs0/data/anzellos/data/forrest/derivatives/fmriprep/'
main_out_dir = '/gsfs0/data/anzellos/data/forrest/half_output_global_compcorr_pc3_v3_component/'
...
for sub_1_index in range(0, len(all_subjects)):
	for sub_2_index in range(0, len(all_subjects)):
		...
		sub_1_data_dir = work_dir + sub_1 + '_complete/' + sub_1 + '_deglobal_compcorr_normalized_demean/' 
		sub_2_data_dir = work_dir + sub_2 + '_complete/' + sub_2 + '_deglobal_compcorr_normalized_demean/' 
		...
		for mask_1_index in range(0, len(all_masks)):
			for mask_2_index in range(0, len(all_masks)):
				...
				for this_run in range(1, total_run + 1):
					...
					test_1_dir = sub_1_data_dir + sub_1 + '_' + mask_1 + '_run_' + str(this_run) + '_deglobal_compcorr_normalized.npy'
					test_2_dir = sub_2_data_dir + sub_2 + '_' + mask_2 + '_run_' + str(this_run) + '_deglobal_compcorr_normalized.npy'
					...
					for run in it.chain(range(1, this_run), range(this_run + 1, total_run + 1)):
						if first_flag:
							train_1 = np.load(sub_1_data_dir + sub_1 + '_' + mask_1 + '_run_' + str(run) + '_deglobal_compcorr_normalized.npy')
							train_2 = np.load(sub_2_data_dir + sub_2 + '_' + mask_2 + '_run_' + str(run) + '_deglobal_compcorr_normalized.npy')
							first_flag = False
						else:
							train_1 = np.concatenate((train_1, np.load(sub_1_data_dir + sub_1 + '_' + mask_1 + '_run_' + str(run) + '_deglobal_compcorr_normalized.npy')))
							train_2 = np.concatenate((train_2, np.load(sub_2_data_dir + sub_2 + '_' + mask_2 + '_run_' + str(run) + '_deglobal_compcorr_normalized.npy')))
