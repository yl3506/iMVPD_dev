...
work_dir = '/mindhive/saxelab3/anzellotti/forrest/fmriprep/'
all_subjects = ['sub-01', 'sub-02', 'sub-03', 'sub-04', 'sub-05', 'sub-09', 'sub-10', 'sub-14', 'sub-15', 'sub-16', 'sub-17', 'sub-18', 'sub-19', 'sub-20']
all_masks_dir = '/mindhive/saxelab3/anzellotti/forrest/fmriprep/'
all_masks = ['rPC.nii.gz', 'rPS.nii.gz', 'rTOS_roi.nii.gz'] # general mask
...
for i in range(0, len(all_subjects)): # iterate through all subjects
	...
	for j in range(0, len(all_masks)):
		...
		for x in range(max_x - 4, max_x + 5):
			for y in range(max_y - 4, max_y + 5):
				for z in range(max_z - 4, max_z + 5):
					if temp_mask[x, y, z] == 1 and anat_data_bin[x, y, z] == 1 and zstat4_data[x, y, z] in max_list:
						final_mask[x, y, z] = 1
