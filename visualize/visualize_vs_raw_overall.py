...
out_dir = main_out_dir + 'vs_cos_global_compcorr_pc3_noATL_v3.png'
...
for sub_1_index in range(0, len(all_subjects)):
	for sub_2_index in it.chain(range(0, sub_1_index), range(sub_1_index + 1, len(all_subjects))):
		...
		data_cross_dir = sub_dir + sub_1 + '_to_' + sub_2 + '_raw_ratio_chart_noATL.npy'
		# load data_cross
...
for sub_index in range(0, len(all_subjects)):
	...
	data_within_dir = sub_dir + subject + '_to_' + subject + '_raw_ratio_chart_noATL.npy'
	# load data_within
...
plt.title('within-cross var expl raw, cos+glob+compcorr, pc=3', y=title_y) # set title
